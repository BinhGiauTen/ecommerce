				...
				<div class="row align-items-center">
					<div class="col-md-8">
						<h4 class="font-20 weight-500 mb-10 text-capitalize">
							<c:choose>
								<c:when test="${not empty a}">Sửa tài khoản</c:when>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="card-box" style="padding: 20px">
						<form action="/Ecommerce/AddAccount" method="post" id="form-1">
							<c:if test="${not empty a}">
								<input type="hidden" name="accountId"
									value="${a.accountId}">
							</c:if>
							<div class="form-group">
								<label for="user_name">Tên tài khoản</label> <input type="text"
									class="form-control" id="user_name"
									value="${a != null ? a.userName : ''}"
									name="user_name"> <span class="form-message"></span>
							</div>
							<div class="form-group">
								<label for="phone">Số điện thoại</label> <input type="text"
									class="form-control" id="phone"
									value="${a != null ? a.phone : ''}" name="phone">
								<span class="form-message"></span>
							</div>
							<div class="form-group">
								<label for="email">Email</label> <input type="text"
									class="form-control" id="email"
									value="${a != null ? a.email : ''}" name="email">
								<span class="form-message"></span>
							</div>
							<div class="form-group">
								<label for="password">Mật khẩu</label> <input type="password"
									class="form-control" id="password"
									value="${a != null ? a.password : ''}"
									name="password"> <span class="form-message"></span>
							</div>
							<div class="form-group">
								<label>Quyền hạn</label>
								<div>
									<input type="radio" id="admin" name="is_admin" value="true">
									<label for="admin">Admin</label>
								</div>
								<div>
									<input type="radio" id="user" name="is_admin" value="false"
										checked> <label for="user">User</label>
								</div>
							</div>
	...
	<script>
		document.addEventListener("DOMContentLoaded", function() {
			Validator({
				...
				rules : [ Validator.isRequired('#user_name'),
